...
setFrameMass(mdl,1,Mb,[mdl.Lb/2 0 mdl.Hb/2]',inertiaBox(Mb,mdl.Lb,mdl.Wb,mdl.Hb));
...
% 'rol'    'pit'    'yaw'    'x'    'y'    'z'    'steerL'    'steerR'    'FL'    'FR'    'BL'    'BR'
disp(stateNames(mdl));
...
if nargout > 3
    ...
    if 0 %load from .fig file (faster) 
        ...
        if use_vrml 
            ...
            C = 'none';
%             C = [1 1 1]/5;
            makeBoxHgt(anim.h_hgt(1),mdl.Lb,mdl.Wb,mdl.Hb,[mdl.Lb/2,0,mdl.Hb/2]',C);
...
%             saveHgt(anim,fig_filename)
    ...
    anim.ax_center = [mdl.Lb/2 0 mdl.Hb/2]';
    d = L;
