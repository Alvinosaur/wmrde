function[] = genmprim_unicycle_circular_non_uniform (outfilename, resolution, errmin, rmin_m, xbounds, ybounds)
...
% errmin = 0.05; %0.05;  half of resolution so should be on actual grid, or could be equal to resolution if adjacent cells are good enough too
% rmin_m = 7.3; %1.0; 
...
function[] = gen_base_prims(angle_ind, prim_ind, xmin0, xmax0, ymin0, ymax0)
    ...
    for i=1:imax
        count = 0; 
        % imax = 2, (i=1)forward, (i=2)backward
        if (i == 1)
            xmn0 = xmin0;
            xmx0 = xmax0;
            ymn0 = ymin0;
